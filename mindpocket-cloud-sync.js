...
(function () {
  ...
  function loadCfg() {
    ...
    c.owner = FIXED.owner;
    c.repo = FIXED.repo;
    c.branch = c.branch || FIXED.branch;
    c.path = c.path || FIXED.path;
    c.mode = c.mode || 'github';
    return c;
  }
  ...
  /* ----------------------------- 浮层 UI ----------------------------- */
  var btnEl = null; // 持有按钮引用，避免切 tab 被移出文档后 getElementById 找不到
  function setBadge(s) {
    var b = btnEl; if (!b) return;
    b.textContent = s === 'ok' ? '☁ 已同步' : (s === 'err' ? '☁ 同步异常' : '☁ 云同步');
    b.style.background = s === 'err' ? '#ff6b6b' : '#2ecc71';
  }
  // 找到「统计」页的容器（<div class="page"> 内含 <h2 class="page-title">统计</h2>）
  ...
  function buildUI() {
    ...
    var btn = document.createElement('button'); btn.id = 'mp-sync-btn'; btn.textContent = '☁ 云同步'; btn.title = 'MindPocket 云同步设置';
    btnEl = btn;
    ...
    modal.innerHTML = '<div id="mp-sync-box">' +
      ...
      '  <label><b>GitHub PAT</b>（唯一需要你填的，细粒度、仅本仓库 Contents:rw）</label>' +
      '  <input id="mp-c-token" placeholder="github_pat_...">' +
      '</div>' +
    ...
    btn.onclick = function () {
      var c = loadCfg();
      // 首次打开默认勾选「启用同步」，用户只需填 PAT 即可保存
      $('#mp-c-en').checked = (c.enabled === undefined) ? true : !!c.enabled;
    ...
    $('#mp-c-save').onclick = function () {
      ...
      if (c.enabled) { setBadge('ok'); pullRemote(); }
      else setBadge('idle');
    };
    if (loadCfg().enabled) setBadge('ok');
  }
  // 把按钮挂到「统计」页右上角；不在统计 tab 时自动移除（兼容 tab 用 v-if 销毁/重建）
